...
class plgAjaxJoomshoppingajaxnewpost extends JPlugin {
    ...
    function onAjaxJoomshoppingajaxnewpost($text){
        ...
        if ($type==1) {
            $rows = json_decode(file_get_contents('http://ws.dexline.com.ua:8888/newpost/SkladList?city_ref='.$ref));
        }
        if ($type==2) {
            $rows = json_decode(file_get_contents('http://ws.dexline.com.ua:8888/newpost/StreetsList?city_ref='.$ref));
        }
