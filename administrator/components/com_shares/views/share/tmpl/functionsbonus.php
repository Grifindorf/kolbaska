                        ...
                        <div class="category" id="category<?php echo $res->category_id; ?>">
                            ...
                            <div class="bonusproduct span7">Добавить бонусный товар:<br><input type="text" block="category" data="<?php echo $res->category_id; ?>" id="searchbonusproduct">
                                <?php foreach ( json_decode($res->bonusproducts) as $bonid=>$bonprod ) {
                                    $sortarray = json_decode($res->ordering, true);
                                    ...
                                    <div class="product" id="product<?php echo $bonid; ?>">
                                        ...
                                        <a onclick="delbonusproduct(<?php echo $bonid; ?>,<?php echo $res->category_id; ?>,'category')" class="actiontodo">удалить</a>
                                        ...
                                        <div id="bonusproductsort">Сортировка: <input type="text" name="jform[selectedbonusproductprice][sort][<?php echo $res->category_id; ?>][<?php echo $bonid; ?>]" value="<?php echo $sortarray[$res->category_id][$bonid][0]; ?>"></div>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                                ...
                                <?php foreach (json_decode($res->category_id) as $rescategory) {
                                    ...
                                    <div class="category" id="category<?php echo $rescategory; ?>">
                                        <div class="maincategory span5">
                                            ...
                                            <a onclick="delbrendcategory(<?php echo $rescategory; ?>,<?php echo $res->brend_id; ?>)" class="actiontodo">удалить</a>
                                        ...
                                        <div class="bonusproduct span7">
                                            ...
                                            <?php foreach ( json_decode($res->bonusproducts)->$rescategory as $bonid=>$bonprod ) {
                                                ...
                                                <div class="product" id="product<?php echo $bonid; ?>">
                                                    ...
                                                    <a onclick="delbonusproduct(<?php echo $bonid; ?>,<?php echo $rescategory; ?>,'brend',<?php echo $res->brend_id; ?>)" class="actiontodo">удалить</a>
                                                    <div id="bonusproductprice">Цена бонусного товара: <input type="text" name="jform[selectedbonusproductprice][<?php echo $res->brend_id; ?>][<?php echo $rescategory; ?>][<?php echo $bonid; ?>][]" value="<?php echo $bonprod[0]; ?>"></div>
                                                    <div id="bonusproductsort">Сортировка: <input type="text" name="jform[selectedbonusproductprice][sort][<?php echo $res->brend_id; ?>][<?php echo $rescategory; ?>][<?php echo $bonid; ?>]" value="<?php echo $sortarray[$rescategory][$bonid][0]; ?>"></div>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    </div>
                    ...
                    <?php foreach ($result as $res) {
                        ...
                        <div class="product" id="product<?php echo $res->product_id; ?>">
                            <div class="mainproduct span5">
                                ...
                                <div id="productsort">Сортировка: <input type="text" name="jform[productsort][sort][<?php echo $res->product_id; ?>]" value="<?php echo $res->mainordering; ?>"></div>
                            ...
                            <div class="bonusproduct span7">Добавить бонусный товар:<br><input type="text" block="product" data="<?php echo $res->product_id; ?>" id="searchbonusproduct">
                                <?php foreach ( json_decode($res->bonusproducts) as $bonid=>$bonprod ) {
                                    ...
                                    <div class="product" id="product<?php echo $bonid; ?>">
                                        ...
                                        <a onclick="delbonusproduct(<?php echo $bonid; ?>,<?php echo $res->product_id; ?>,'product')" class="actiontodo">удалить</a>
                                        ...
                                        <div id="bonusproductsort">Сортировка: <input type="text" name="jform[selectedbonusproductprice][sort][<?php echo $res->product_id; ?>][<?php echo $bonid; ?>]" value="<?php echo $sortarray[$bonid]; ?>"></div>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
...
<script>
    jQuery(document).ready(function(){
                ...
                jQuery.ajax({
                    ...
                    complete: function(response){
                        ...
                        jQuery(response.responseJSON['data'][0]).each(function(){
                            ...
                            '<div class="bonusproduct" style="display:none;">Добавить бонусный товар:<br/><input type="text" id="searchbonusproduct" data="'+this.id+'" data-brendid="'+parentblockid+'" block="category"/></div>'+
                            '</div>');
                        });
        ...
        jQuery('input#searchbonusproduct').live('keyup', function (e) {
            ...
            var brendid = jQuery(this).attr('data-brendid');
            var searchtext = jQuery(this).val();
            var searchlength = searchtext.length;
            var dataval = [];
            dataval[0] = searchtext;
                    ...
                    complete: function(response){
                        ...
                        jQuery(response.responseJSON['data'][0]).each(function(){
                            if (brendid) {
                                ...
                                '<a class="actiontodo" id="addbonusproduct'+this.id+'" onclick="addbonusproduct('+this.id+','+parentblockid+',\'brend\','+brendid+')">добавить</a>'+
                                '</div>');
                            } else {
                                jQuery('#resultselect #browseselect').append('<div id="product'+this.id+'" class="product">'+
                                '<a class="titleproduct" target="_blank" href="'+this.href+'"><span class="title">'+this.title+'</span></a>'+
                                '<a class="actiontodo" id="addbonusproduct'+this.id+'" onclick="addbonusproduct('+this.id+','+parentblockid+',\''+parentblock+'\','+brendid+')">добавить</a>'+
                                '</div>');
                            }
                        });
...
<script>
    function addbrendcategory(id,blockid) {
        ...
        jQuery('#brend'+blockid+' #category'+id+' .maincategory').addClass('span5');
        jQuery('#brend'+blockid+' #category'+id+' .bonusproduct').addClass('span7');
        jQuery('#brend'+blockid+' #category'+id+' .bonusproduct').show();
        jQuery('#brend'+blockid+' #category'+id+' .maincategory').append('<a class="actiontodo" onclick="delbrendcategory('+id+','+blockid+')">удалить</a>');
        jQuery('#brend'+blockid+' #category'+id+' .maincategory').append('<input type="hidden" name="jform[selectedcategory]['+blockid+'][]" value="'+id+'" />');
    }
    function delbrendcategory(id,blockid) {
        jQuery('#brend'+blockid+' #category'+id).remove();
    }
</script>
...
<script>
    function addbonusproduct(id,blockid,block,brendid) {
        if (block=='brend') {
            jQuery('#brend'+brendid+' #category'+blockid+' .bonusproduct').append(jQuery('#product'+id));
            jQuery('#brend'+brendid+' #category'+blockid+' #bonusproductprice').show();
            jQuery('#brend'+brendid+' #addbonusproduct'+id).remove();
            jQuery('#brend'+brendid+' #category'+blockid+' #product'+id).append('<a class="actiontodo" onclick="delbonusproduct('+id+','+blockid+',\''+block+'\','+brendid+')">удалить</a>');
        }else {
            ...
            jQuery('#'+block+''+blockid+' #product'+id).append('<a class="actiontodo" onclick="delbonusproduct('+id+','+blockid+',\''+block+'\')">удалить</a>');
        }
        if (block=='brend') {
            jQuery('#brend'+brendid+' #category'+blockid+' #product'+id).append('<div id="bonusproductprice">Цена бонусного товара: <input type="text" name="jform[selectedbonusproductprice]['+brendid+']['+blockid+']['+id+'][]" value="1"/></div>');
            jQuery('#brend'+brendid+' #category'+blockid+' #product'+id).append('<div id="bonusproductsort">Сортировка: <input type="text" name="jform[selectedbonusproductprice][sort]['+brendid+']['+blockid+']['+id+']" value="0"/></div>');
            jQuery('#brend'+brendid+' #category'+blockid+' #product'+id).append('<input type="hidden" name="jform[selectedbonusproduct]['+brendid+']['+blockid+'][]" value="'+id+'" />');
        }else {
            jQuery('#'+block+''+blockid+' #product'+id).append('<div id="bonusproductprice">Цена бонусного товара: <input type="text" name="jform[selectedbonusproductprice]['+blockid+']['+id+'][]" value="1"/></div>');
            jQuery('#'+block+''+blockid+' #product'+id).append('<div id="bonusproductsort">Сортировка: <input type="text" name="jform[selectedbonusproductprice][sort]['+blockid+']['+id+']" value="0"/></div>');
            jQuery('#'+block+''+blockid+' #product'+id).append('<input type="hidden" name="jform[selectedbonusproduct]['+blockid+'][]" value="'+id+'" />');
        }
    }
    function delbonusproduct(id,parentid,block,brendid) {
        if (block=='product') {
            jQuery('#product'+parentid+' #product'+id).remove();
        }
        if (block=='category') {
            jQuery('#category'+parentid+' #product'+id).remove();
        }
        if (block=='brend') {
            jQuery('#brend'+brendid+' #category'+parentid+' #product'+id).remove();
        }
    }
</script>
