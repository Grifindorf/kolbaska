...
class JshoppingControllerProviders extends JControllerLegacy{
    ...
    function edit() {
        ...
        $manufacturer = JSFactory::getTable('provider', 'jshop');
    ...
    function save(){
        ...
        $db = JFactory::getDBO();
        $man = JSFactory::getTable('provider', 'jshop');
        ...
        foreach($languages as $lang){
            ...
            if ($post['alias_'.$lang->language]=="") $post['alias_'.$lang->language] = $post['name_'.$lang->language];
        ...
        if (!$man->bind($post)) {
            ...
            $this->setRedirect("index.php?option=com_jshopping&controller=providers");
            return 0;
        }
        ...
        if ($this->getTask()=='apply'){
            $this->setRedirect("index.php?option=com_jshopping&controller=providers&task=edit&man_id=".$man->manufacturer_id);
        }else{
            $this->setRedirect("index.php?option=com_jshopping&controller=providers");
        }
    ...
    function remove(){
        ...
        foreach ($cid as $key => $value) {
            $manuf = JSFactory::getTable('provider', 'jshop');
            $manuf->load($value);
            $manuf->delete();
        ...
        $this->setRedirect("index.php?option=com_jshopping&controller=providers", implode("</li><li>",$text));
    }
    ...
    function publishManufacturer($flag) {
        ...
        foreach ($cid as $key => $value) {
            $query = "UPDATE `#__jshopping_providers`
    ...
    function saveorder(){
        ...
        foreach ($cid as $k=>$id){
            $table = JSFactory::getTable('provider', 'jshop');
            $table->load($id);
