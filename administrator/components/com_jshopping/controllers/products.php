...
class JshoppingControllerProducts extends JControllerLegacy{
    ...
    function edit(){
        ...
        $_provider =JSFactory::getModel('providers');
        $provs = $_provider->getList();
        $provs = array_merge($manuf1, $provs);
        ...
        $lists['providers'] = JHTML::_('select.genericlist', $provs,'product_provider_id','class = "inputbox" size = "1"','manufacturer_id','name',$product->product_provider_id);
    ...
    function save(){
        ...
        if($post['product_ean']==''){
            $query = "SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'kolbaska' AND TABLE_NAME  = 'wzjit_jshopping_products'";
            $db->setQuery($query);
            $result_ean = $db->loadResult();
            $post['product_ean'] = 2053000 + $result_ean;
        }
