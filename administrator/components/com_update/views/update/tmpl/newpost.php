...
$file_city = file_get_contents('http://ws.dexline.com.ua:8888/newpost/CitiesList');

$query = "TRUNCATE TABLE `#__newpostcity`";
$db->setQuery($query);
$db->execute();

foreach (json_decode($file_city) as $city) {
    $query = "INSERT INTO `#__newpostcity`(`id`, `ref`, `name`) VALUES (".$db->quote($city->id).",".$db->quote($city->ref).",".$db->quote($city->nameRu).")";
    $db->setQuery($query);
    $db->execute();
}

mail('v.repij@gmail.com','Обновление городов новой почты',count(json_decode($file_city)));
