...
unlink('/home/www/dexline.com.ua/pn.xml');

$yaxml = fopen("/home/www/dexline.com.ua/pn.xml", "w");
...
$query = "SELECT jp.*,jpc.category_id,jc.`name_ru-RU` as category_name,jm.`name_ru-RU` as manufacturer_name FROM #__jshopping_products as jp
          INNER JOIN #__jshopping_products_to_categories as jpc ON jp.product_id=jpc.product_id
          INNER JOIN #__jshopping_categories as jc ON jc.category_id=jpc.category_id
...
foreach ($results as $res) {
    ...
    $txt .= "<typePrefix>".htmlspecialchars(strip_tags($res->category_name))."</typePrefix>\r\n";
...
mail('v.repij@gmail.com','Обновление прайса прайс навигатор',count($products));
mail('price@dexline.com.ua','Обновление прайса прайс навигатор',count($products));