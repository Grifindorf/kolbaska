...
<nav class="navbar<?php echo $template_is_light ? '' : ' navbar-inverse'; ?> navbar-fixed-top">
    <div class="navbar-inner">
        <div class="container-fluid">
            ...
            <a class="admin-logo <?php echo ($hidden ? 'disabled' : ''); ?>" <?php echo ($hidden ? '' : 'href="' . $this->baseurl . '"'); ?>><span class="icon-home-2 cpanel"></span></a>
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <?php if (!$hidden) : ?>
                                ...
                                <li class="divider"></li>
                                <li>
                                    <a href="index.php?option=com_admin&task=profile.edit&id=<?php echo $user->id; ?>"><?php echo JText::_('TPL_CUSTOMSTYLES_EDIT_ACCOUNT'); ?></a>
                                </li>
                                <li class="divider"></li>
                                <li class="">
                                    <a href="<?php echo JRoute::_('index.php?option=com_login&task=logout&' . JSession::getFormToken() . '=1'); ?>"><?php echo JText::_('TPL_CUSTOMSTYLES_LOGOUT'); ?></a>
                                </li>
                            <?php endif; ?>
                        </ul>
                    </li>
...
<link rel="stylesheet" type="text/css" href="/administrator/templates/customstyles/css/jquery.growl.css" media="all">
<script type="text/javascript" src="/administrator/templates/customstyles/js/jquery.growl.js"></script>
...
<script>
    ...
    function sendBrowserNotification(title, text, icon) {
        if(icon == '' || icon == null || icon == undefined){
            icon = '/template/customstyles/images/shopping7.png';
        }
    ...
    jQuery(document).ready(function(){
        function CheckNewOrder(){
            var date = new Date();
            var date2 = parseInt(new Date().getTime()/1000);
            var cookie = getCookie("nowOrderLastUpdate") != undefined ? getCookie("nowOrderLastUpdate") : 0;
            cookie = parseInt(cookie);
            date = date.getHours() + ':' + ('0' + (date.getMinutes())).slice(-2);
            if(cookie + 5 < date2){
                console.log("cookie + 5 = " + (cookie + 5) + "; date2 = " + date2);
                jQuery.ajax({
                    type: "POST",
                    url: "/index.php?option=com_ajax&plugin=neworder&format=json",
                    //data: {"q_search": search},
                    cache: false,
                    success: function(result){
                        var data = result.data[0];
                        if(data.replace(/\D+/, '') != data ){
                            //console.log(data);
                        }else if(nowOrder != '' && nowOrder != data && data != 0 && data != parseInt(getCookie("nowOrder"))){
                            document.cookie = 'nowOrder=' + data;
                            sendBrowserNotification('Новый заказ!', "В " + date + " к нам поступил новый заказ!  \nНомер заказа: " + data, '');
                            jQuery.growl.notice({
                                title: "Новый заказ!",
                                message: "Сегодня, в " + date + " к нам поступил новый заказ! <br>Номер заказа: " + data + '<audio src="/administrator/templates/customstyles/images/icq.wav" autoplay="true" preload="true"></audio>'
                            });
                        }
                        if(data.replace(/\D+/, '') == data && data != 0){
                            nowOrder = data;
                            document.cookie = 'nowOrderLastUpdate=' + date2;
                        }
                    }
                });
            }
