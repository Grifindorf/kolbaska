...
function disable(){
    Array.from(arguments).each(function(el){
        if($(el)) $(el).getParent().getParent().addClass('noDisplay');
    });
}
function enable(){
    Array.from(arguments).each(function(el){
        if($(el)) $(el).getParent().getParent().removeClass('noDisplay');
    });
}
window.addEvent('load',function(){
    jQuery('#jform_params_products_source').chosen().change(function(){
        var val = jQuery('#jform_params_products_source').chosen().val();
        switch(val){
    ...
    jQuery('#jform_params_products_source').chosen().trigger('change');
