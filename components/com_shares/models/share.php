        ...
        if ($datavr['id']) {
            ...
            if ($datavr['type']!=0) {
                ...
                if ($datavr['type']==4) {
                    ...
                    $query = "SELECT * FROM `#__shares_products` WHERE shares_id = ".$datavr['id']." ";
                    $db->setQuery($query);
                    $result = $db->LoadAssocList();

                    if ($result) {
                        $query = "UPDATE `#__shares_products` SET `typesearch` = ".$db->quote($datavr['typesearch']).", `category_id` = ".$db->quote(json_encode($datavr['typecategory'])).", `brend_id` = ".$db->quote(json_encode($datavr['typebrend'])).", `product_id` = ".$db->quote(json_encode($datavr['typeproduct'])).", `singlebonus` = ".$db->quote($datavr['typebonusproduct']).", `singlebonusprice` = ".$db->quote($datavr['typebonusproductprice'])." WHERE shares_id = ".$datavr['id']." ";
                        $db->setQuery($query);
                        $db->query();
                    } else {
                        $query = "INSERT INTO `#__shares_products` (`shares_id`, `typesearch`, `category_id`, `brend_id`, `product_id`, `singlebonus`, `singlebonusprice`) VALUES (".$datavr['id'].", ".$db->quote($datavr['typesearch']).", ".$db->quote(json_encode($datavr['typecategory'])).", ".$db->quote(json_encode($datavr['typebrend'])).", ".$db->quote(json_encode($datavr['typeproduct'])).", ".$db->quote($datavr['typebonusproduct']).", ".$db->quote($datavr['typebonusproductprice'])." )  ";
                        $db->setQuery($query);
                        $db->query();
                    }
