...
function jshoppingBuildRoute(&$query){
    ...
    $vr_itemid = $query['Itemid'];
    if ($query['controller'] == 'product') {
        $menu->getItem($query['Itemid'])->alias = 'product';
        $shim->shop = 162;
        $query['Itemid'] = 162;
    }elseif ($query['controller'] == 'cart') {
        $menu = $app->getMenu();
        $shim = shopItemMenu::getInstance();
        $query['Itemid'] = 163;
    }else{
    ...
    /*if ($controller=="product" && $query['task']=="view" && $query['product_id']){
        $prodalias = JSFactory::getAliasProduct();
        $catalias = JSFactory::getAliasCategory();
        if (isset($categoryitemidlist[$query['category_id']]) && isset($prodalias[$query['product_id']])){
            unset($query['controller']);
            ...
            $segments[] = $prodalias[$query['product_id']];
            unset($query['product_id']);
        }elseif (isset($prodalias[$query['product_id']])){
            //$segments[] = $catalias[$query['category_id']];
            $segments[] = $prodalias[$query['product_id']];
            unset($query['controller']);
            unset($query['task']);
...
function jshoppingParseRoute($segments){
    ...
    //if ($menuItem->query['controller']=="product"){die;}
    ...
    if ($segments[0] && !in_array($segments[0], $reservedFirstAlias)){
        ...
        if ($category_id && ( $segments[1]=="" or $manufacturer_id ) ){
        ...
        if ($category_id && ( $segments[1]!="" or $segments[1]!=$manufacturer_id )){
        ...
        if (!$category_id && $segments[1]==""){
            ...
            if ($product_id){
                $category_id = JSFactory::getProductCategory($product_id);
                $vars['controller'] = "product";
                $vars['task'] = "view";
                $vars['category_id'] = $category_id;
                $vars['product_id'] = $product_id;
