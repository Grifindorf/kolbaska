    ...
    function loadPriceAndCountProducts(){
        ...
        if (count($this->products)){
            foreach($this->products as $prod){
                ...
                $this->price_product_opt += $prod['price'] * $prod['quantity'];
                $this->price_product_rozn += $prod['price_rozn'] * $prod['quantity'];
        ...
        if($this->price_product_rozn<500){
            ...
            $this->type_cart_price = 1;
        }else{
            $this->type_cart_price = 0;
            $this->price_product = $this->price_product_rozn;
        }
    ...
    function loadProducts(){
        ...
        if(count($this->products)>0){
            foreach($this->products as $key=>$product){
                ...
                $products[$product['product_id']]['key'] = $key;
            }
        }
        return $products;
    }
    ...
    */
    ...
    function add($product_id, $quantity, $attr_id, $freeattributes, $additional_fields = array(), $usetriggers = 1, &$errors = array(), $displayErrorMessage = 1){
        ...
        if ($updateqty){
            foreach ($this->products as $key => $value){
                if ($value['product_id'] == $product_id && $value['attributes'] == $attr_serialize && $value['freeattributes']==$free_attr_serialize){
                    ...
                    $this->products[$key]['price'] = $product->getPrice($this->products[$key]['quantity'], 1, 1, 1, $this->products[$key]);
                    $this->products[$key]['price_rozn'] = $product->getPriceRozn($this->products[$key]['quantity'], 1, 1, 1, $this->products[$key]);
                    if ($jshopConfig->cart_basic_price_show){
                        $this->products[$key]['basicprice'] = $product->getBasicPrice();
                    }
        ...
        if ($new_product){
            ...
            $temp_product['price_rozn'] = $product->getPriceRozn($quantity, 1, 1, 1, $temp_product);
            ...
            $this->products[$temp_product['product_id']] = $temp_product;
        }
    ...
    function addKassa($product_id, $quantity){
        $jshopConfig = JSFactory::getConfig();
        if ($quantity <= 0){
            $errors['100'] = 'Error quantity';
            if ($displayErrorMessage){
