...
class JSFactory{
    ...
    public static function getAliasCategoryVR($langvr){
        static $alias;
        if (!is_array($alias)){
            $db = JFactory::getDBO();
            $lang = JSFactory::getLang();
            $dbquery = "select category_id as id, `alias_".$langvr."` as alias from #__jshopping_categories where `alias_".$langvr."`!=''";
            $db->setQuery($dbquery);
            $rows = $db->loadObjectList();
            $alias = array();
            foreach($rows as $row){
    ...
    static $alias;
        if (!is_array($alias)){
            $db = JFactory::getDBO();
            $lang = JSFactory::getLang();
            $dbquery = "select manufacturer_id as id, `alias_ru-RU` as alias from #__jshopping_manufacturers where `alias_ru-RU`!=''";
            $db->setQuery($dbquery);
            $rows = $db->loadObjectList();
            $alias = array();
            foreach($rows as $row){
    ...
    public static function getNameManufacturer(){
        static $alias;
        if (!is_array($alias)){
            $db = JFactory::getDBO();
            $lang = JSFactory::getLang();
            $dbquery = "select manufacturer_id as id, `name_ru-RU` as name from #__jshopping_manufacturers where `name_ru-RU`!=''";
            $db->setQuery($dbquery);
            $rows = $db->loadObjectList();
            $alias = array();
            foreach($rows as $row){
                $alias[$row->id] = $row->name;
            }
            unset($rows);
        }
        return $alias;
    ...
    public static function getAliasProductVR($langvr){
        static $alias;
        if (!is_array($alias)){
            $db = JFactory::getDBO();
            $lang = JSFactory::getLang();
            $dbquery = "select product_id as id, `alias_".$langvr."` as alias from #__jshopping_products where `alias_".$langvr."`!=''";
            $db->setQuery($dbquery);
            $rows = $db->loadObjectList();
            $alias = array();
            foreach($rows as $k=>$row){
    ...
    public static function getProductCategory($id){
        ...
        if (!is_array($alias)){
            $db = JFactory::getDBO();
            $lang = JSFactory::getLang();
            $dbquery = "select category_id as id from #__jshopping_products_to_categories where `product_id`='".$id."'";
            $db->setQuery($dbquery);
            $rows = $db->loadResult();
        }
        return $rows;
    ...
    public static function getAllProductExtraFieldValueClass(){
        static $list;
        if (!is_array($list)){
            $productfieldvalue = JSFactory::getTable('productfieldvalue', 'jshop');
            $list = $productfieldvalue->getAllListClass(1);
        }
        return $list;
    }
