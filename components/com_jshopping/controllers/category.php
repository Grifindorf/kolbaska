...
class JshoppingControllerCategory extends JControllerLegacy{
    ...
    function view(){
        ...
        $cart = $cart = JSFactory::getModel('cart', 'jshop');
        $cart->load();

        $cart_count_products = $cart->getCountProduct();
        ...
        if(!$manufacturer_ids){
            $manufacturer_ids = $jinput->get('manufacturer_id','','STRING');
        }
        ...
        if ( $manufacturer_ids ) {
            ...
            foreach (explode(',',$manufacturer_ids) as $manurl) {
                $active_manufacturers[] = $manalias[$manurl];
            }
        }
        if ($filters_url) {
            ...
            foreach ($explode_tmp as $exp) {
                ...
                if ($extra_field_active_id) {
                    $query = "SELECT `name_ru-RU` as name FROM `#__jshopping_products_extra_fields`
                          WHERE id = ".$extra_field_active_id."  ";
                    $db->setQuery($query);
                    $result_filter = $db->LoadResult();
                    $active_extra_fields[$extra_field_active_id]['name'] = $result_filter;
                    foreach (explode(',',array_shift($explode_tmp_val)) as $values) {
                        $query = "SELECT `name_ru-RU` as name FROM `#__jshopping_products_extra_field_values`
                          WHERE id = ".$values."  ";
                        $db->setQuery($query);
                        $result_filter = $db->LoadResult();
                        $active_extra_fields[$extra_field_active_id]['values'][] = $result_filter;
                    }
                }
            }
        }
        ...
        if ($active_extra_fields) {
            foreach ($active_extra_fields as $key=>$extra) {
                $category_meta_description[] = $extra['name'].': '.implode(', ',$extra['values']);
            }

        }
        if ($active_extra_fields||$manufacturer_ids) {
            if($manufacturer_ids){
                $category->meta_description = $category_brend_meta_description;
                $category->meta_title = $category_brend_meta_description;
            }
            if($active_extra_fields){
                $category->meta_description .= implode('; ',$category_meta_description).'. '.$category->meta_description;
                $category->meta_title .= implode('; ',$category_meta_description).'. '.$category->meta_title;
            }
        }
        ...
        $products_in_cart = $cart->loadProducts();
        if(count($products_in_cart)>0){
            foreach($products as $product){
                if($products_in_cart[$product->product_id]){
                    $product->in_cart = 1;
                    $product->in_cart_key = $products_in_cart[$product->product_id]['key'];
                    $product->in_cart_qtty = $products_in_cart[$product->product_id]['quantity'];
                }
            }
        }
        ...
        $view->assign('count_products_in_cart', $cart_count_products);
        ...
        $view->assign('noimage', $jshopConfig->noimage);
        $view->assign('category', $category);
        $view->assign('category_id', $category->category_id);
