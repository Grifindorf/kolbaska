...
class JshoppingControllerCheckout extends JControllerLegacy{
    ...
    function step5save(){
        ...
        if ($pm_method->payment_type == 1){
            ...
            if ($jshopConfig->send_order_email){
                //$checkout->sendOrderEmail($order->order_id); TODO: error has occured in this functio
            }
        }
    ...
    function step5saveAjax(){
        ...
        $cart = JSFactory::getModel('cart', 'jshop');
        $cart->load();
        $order_total = $cart->getPriceProducts();

        $phone = JRequest::getVar('phone');

        $db = JFactory::getDbo();

        $query = "SELECT `order_number` FROM `#__jshopping_orders` ORDER BY `order_id` DESC LIMIT 1";
        $db->setQuery($query);
        $last_id = $db->loadResult();

        $query = "SELECT `order_id` FROM `#__jshopping_orders` ORDER BY `order_id` DESC LIMIT 1";
        $db->setQuery($query);
        $last_id_order = $db->loadResult();

        $session->set("jshop_end_order_id", $last_id_order);
        $session->set("jshop_send_end_form", 0);
        $checkout->setMaxStep(10);

        $tax_ext = 'a:1:{s:5:"19.00";d:0;}';

        foreach ($cart->products as $product) {
            $query = "INSERT INTO `#__jshopping_order_item`
            (`order_id`,`product_id`,`product_ean`,`product_name`,`product_quantity`,`product_item_price`,`thumb_image`,`vendor_id`)
            VALUES
            ('".($last_id_order+1)."','".$product['product_id']."','".$product['ean']."','".$product['product_name']."','".$product['quantity']."','".$product['price']."','".$product['thumb_image']."',1)";
            $db->setQuery($query);
            $results = $db->execute();
        }

        $query = "INSERT INTO `#__jshopping_order_history`
        (`order_id`,`order_status_id`,`status_date_added`,`customer_notify`)
        VALUES
        ('".($last_id_order+1)."',1,'".date('Y-m-d H:i:s')."',1)";
        $db->setQuery($query);
        $results = $db->execute();

        $query = "INSERT INTO `#__jshopping_orders`
        (`order_number`,`user_id`,`order_total`,`order_subtotal`,`order_tax_ext`,`shipping_tax`,`shipping_tax_ext`,`payment_tax`,`payment_tax_ext`,`package_tax_ext`,`currency_code`,`currency_code_iso`,
        `currency_exchange`,`order_status`,`order_created`,`order_date`,`order_m_date`,`shipping_method_id`,`payment_method_id`,`phone`,`d_phone`,`display_price`,`vendor_id`,`lang`,`product_stock_removed`)
        VALUES
        ('".str_pad(($last_id_order+1), 8, '0', STR_PAD_LEFT)."','-1','".$order_total."','".$order_total."','a:0:{}',19,'".$tax_ext."',19,'".$tax_ext."','".$tax_ext."','грн','UAH',1,1,1,'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."',
        1,1,'".$phone."','".$phone."',1,1,'ru-RU',1)";
        $db->setQuery($query);
        $results = $db->execute();

        $link = SEFLink('index.php?option=com_jshopping&controller=checkout&task=finish');

        print $link;
        die;
    }

    function step5saveAjaxKassa(){
        $session = JFactory::getSession();
        ...
        foreach ($cart->products as $product) {
            ...
            $db->setQuery($query);
            $results = $db->execute();
            $query ="UPDATE `#__jshopping_products` SET `product_quantity` = `product_quantity` - '".$product['quantity']."' WHERE `product_id` = '".$product['product_id']."' ";
            $db->setQuery($query);
            $db->execute();
        }

        $query = "INSERT INTO `#__jshopping_order_history`
        (`order_history_id`,`order_id`,`order_status_id`,`status_date_added`,`customer_notify`)
        VALUES
        ('".($last_id_order+1)."','".($last_id_order+1)."',1,'".date('Y-m-d H:i:s')."',1)";
        $db->setQuery($query);
        $results = $db->execute();
        ...
        `currency_exchange`,`order_status`,`order_created`,`order_date`,`order_m_date`,`shipping_method_id`,`payment_method_id`,`phone`,`d_phone`,`display_price`,`vendor_id`,`lang`,`product_stock_removed`)
        VALUES
        ('".str_pad(($last_id+1), 8, '0', STR_PAD_LEFT)."','-1','".$order_total."','".$order_total."','a:0:{}',19,'".$tax_ext."',19,'".$tax_ext."','".$tax_ext."','грн','UAH',1,1,1,'".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."',
        1,1,'".$phone."','".$phone."',1,1,'ru-RU',1)";
        $db->setQuery($query);
        $results = $db->execute();
