...
class jshopProvider extends JTableAvto{

    function __construct(&$_db){
        parent::__construct('#__jshopping_providers', 'manufacturer_id', $_db);
    ...
    function getAllManufacturers($publish = 0, $order = "ordering", $dir ="asc" ) {
        ...
        $query = "SELECT manufacturer_id, manufacturer_url, manufacturer_logo, manufacturer_publish, `".$lang->get('name')."` as name, `".$lang->get('description')."` as description,  `".$lang->get('short_description')."` as short_description
				  FROM `#__jshopping_providers` $query_where ORDER BY ".$orderby." ".$dir;
        $db->setQuery($query);
        $list = $db->loadObjectList();

        foreach($list as $key=>$value){
            $list[$key]->link = SEFLink('index.php?option=com_jshopping&controller=provider&task=view&manufacturer_id='.$list[$key]->manufacturer_id);
        }
        return $list;
    }
