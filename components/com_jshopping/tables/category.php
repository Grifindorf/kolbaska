...
class jshopCategory extends JTableAvto{
    ...
    function getSubCategories($parentId, $order = 'id', $ordering = 'asc', $publish = 0) {
        ...
        $query = "SELECT `".$lang->get('name')."` as name, `".$lang->get('titledescr')."` as titledescr,`".$lang->get('description')."` as description,`".$lang->get('short_description')."` as short_description, category_id, category_publish, ordering, category_image FROM `#__jshopping_categories`
    ...
    function getProducts($filters, $order = null, $orderby = null, $limitstart = 0, $limit = 0) {
        ...
        $query = "SELECT $adv_result , `product_provider_id` FROM `#__jshopping_products` AS prod
                  LEFT JOIN `#__jshopping_products_to_categories` AS pr_cat USING (product_id)
                  $adv_from
                  WHERE pr_cat.category_id = '".$this->_db->escape($this->category_id)."' AND prod.product_publish = '1' AND prod.product_original_price > 0 ".$adv_query." ".$order_query;
    ...
    function getCountProducts($filters){
        ...
        $query = "SELECT $adv_result FROM `#__jshopping_products_to_categories` AS pr_cat
                  ...
                  $adv_from 
                  WHERE pr_cat.category_id = '".$this->_db->escape($this->category_id)."' AND prod.product_publish = '1' AND prod.product_original_price > 0 ".$adv_query;
        $this->_db->setQuery($query);
