...
class jshopProductFieldValue extends JTableAvto{
    ...
    function getAllListClass($display = 0){
        $db = JFactory::getDBO();
        $lang = JSFactory::getLang();
        $query = "SELECT id, `".$lang->get("name")."` as name, field_id, class FROM `#__jshopping_products_extra_field_values` order by ordering";
        $db->setQuery($query);
        if ($display==0){
            return $db->loadObjectList();
        }elseif($display==1){
            ...
            $list = array();
            foreach($rows as $k=>$row){
                $list[$row->field_id][$row->id] = $row->class;
                unset($rows[$k]);
            }
            return $list;
        }
