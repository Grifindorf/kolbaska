...
<div class="single-item">
    ...
    <div class="container">
        ...
        <div class="single-item-content <?=($product->in_cart==1 ? 'inCart' : '')?>" id="product-<?=$this->product->product_id?>">
            <div class="row">
                <div class="col-md-4 col-sm-5">
                    <!-- Product image -->
                    <?php foreach($this->images as $k=>$image){?>
                        <img class="img-responsive img-thumbnail" id = "main_image_<?php print $image->image_id?>" src = "<?php print $this->image_product_path?>/<?php print $image->image_name;?>" alt="<?php print htmlspecialchars($image->_title)?>" title="<?php print htmlspecialchars($image->_title)?>" />
                    <?php }?>
                </div>
                <div class="col-md-8 col-sm-7">
                    <!-- Heading -->
                    <h3><?php print $this->product->name?></h3>
                    <div class="row">
                        <div class="col-md-7 col-sm-12">
                            <!-- Single item details -->
                            <div class="item-details">
                                <!-- Paragraph -->
                                <p class="text-justify"><?php print $this->product->description; ?></p>
                            </div>
                            <div class="shopping">
                                <div class="shopping-item">
                                    <div class="prices-category">
                                        <span class="item-price <?=($this->count_products_in_cart > 0 ? 'active' : 'active')?>"><?= formatprice($product->product_price)?>/кг</span>
                                        <span class="item-price <?=($this->count_products_in_cart < 5 ? '' : '')?>">розница - <?= formatprice($product->product_old_price)?>/кг</span>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="quantitychange">
                                                <span class="btn btn-default btn-sm" onclick="changeQtty(<?=$this->product->product_id?>,'minus')">&nbsp;-</span><input type="text" class="quantitycategory" attr-key="<?=$this->product->product_id?>" id="quantity<?=$this->product->product_id?>" value="<?php print $quantity; ?>"><span class="btn btn-default btn-sm" onclick="changeQtty(<?=$this->product->product_id?>,'plus')">+</span>
                                            </div>
                                            <a class="btn btn-danger <?=($product->in_cart==1 ? 'br-green' : 'br-red')?>" onclick="addToCart(<?=$this->product->product_id?>)" href="javascript:void(null)"><?=($product->in_cart==1 ? 'В корзине' : _JSHOP_BUY)?></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<div class="jshop productfull" id="comjshop">
        ...
        <h1><?php if ($this->config->show_product_code){?> <span class="jshop_code_prod">(<?php print _JSHOP_EAN?>: <span id="product_code"><?php print $this->product->getEan();?></span>)</span><?php }?></h1>
